...
def test_basic_functionality():
    ...
    for i, job in enumerate(jobs):
        ...
        print(f'taskname: {job[0]._dtq_task_name}, id: {job_id}, status: {dtq.get_status(job_id)}')
    ...
    for i, job_id in enumerate(jobs_ids):
        ...
        if i == 0:  # add task
            ...
            assert "Hello from Docker!" in str(result['result']), f"Expected echo message in {result['result']}"
...
def test_concurrent_processing():
    ...
    assert completed_count == num_tasks, f"Expected {num_tasks} completed, got {completed_count}"
...
def test_performance():
    ...
    assert correct_results == num_tasks, f"Expected {num_tasks} correct results, got {correct_results}"
...
def wait_for_completion(job_ids, timeout=10, poll_interval=0.5):
    ...
    if remaining_jobs:
        print(f"Warning: {len(remaining_jobs)} jobs did not complete within {timeout} seconds")
